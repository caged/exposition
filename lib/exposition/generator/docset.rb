      ...
      class Generator < Exposition::Generators::Base
        ...
        private
          ...
          def validate_package
            puts blue "==> Validating Package..."
            Open3.popen3 "#{docsetutil} validate #{bundle_root}" do |stdin, stdout, stderr|
              puts stdout.read
            end
          end
          ...
          def dir_for_object(obj)
            case obj
              when ObjCObjects::ObjCClass    then 'Classes'
              when ObjCObjects::ObjCProtocol then 'Protocols'
              when ObjCObjects::ObjCCategory then 'Categories'
            end
          end
      end
    end
